...
export const getMoveResultingState = (
  targetSquare: number,
  playerColor: Color,
  opponentColor: Color,
  currentStateAsString: string
): string => {
  const playerColorStr = playerColor.toString();
  const opponentColorStr = opponentColor.toString();

  let currentState = stateStringToArray(currentStateAsString);

  currentState[targetSquare] = playerColorStr;

  for (const direction of Object.values(DIRECTION_VALUES)) {
    let currentPosition = targetSquare + (direction as unknown as number);
    const flippedSquares = new Array<number>();

    while (
      isInBounds(currentPosition) &&
      currentState[currentPosition] === opponentColorStr
    ) {
      flippedSquares.push(currentPosition);
      currentPosition += direction as unknown as number;
    }

    if (
      isInBounds(currentPosition) &&
      currentState[currentPosition] === playerColorStr
    ) {
      for (const square of flippedSquares) {
        currentState[square] = playerColorStr;
      }
    }
  }

  return stateArrayToString(currentState);
};
...
export function setBoardHeightProperty() {
  ...
  const trySetBoardHeight = async (attempts = 0) => {
    ...
    if (board) {
      ...
      console.log(wHeight)
      console.log(wWidth)
      var minBoardSize = 300;
      var boardSize;
      var componentSize;

      if (wWidth >= 1200) {
        // Adjust width and height to account for gaps and padding
        wWidth = wWidth - 60;
        wHeight = wHeight - 70;

        // Calculate maximum allowable board size
        ...
        // Calculate component size as a proportion of the remaining viewport width
        var remainingWidth = wWidth - boardSize;
